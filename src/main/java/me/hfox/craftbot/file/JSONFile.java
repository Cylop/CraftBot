...
public class JSONFile {
    ...
    public void load() {
        if (file.exists() && file.isDirectory()) {
            Log.warning("Could not load '" + file.getName() + "' because it was a directory");
            return;
        }

        if (!file.exists()) {
            ClassLoader loader = getClass().getClassLoader();
            InputStream stream = loader.getResourceAsStream(file.getName());
            TextFile text = new TextFile(file);
            try {
                file.createNewFile();
                text.addLine(IOUtils.toString(stream));
